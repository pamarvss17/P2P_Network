    ...
    while(check(replyreceived, noofneighbour) || check(replysent, noofneighbour)){
        for (int i = 0; i < noofneighbour; i++) {
            ...
            if ((requestsent[i] == 1 && replyreceived[i] == 0) || (requestreceived[i] == 0)) {
                ...
                if (bytesreceived > 0) {
                    cout << "Received " << bytesreceived << " from " << neighbourids[i] << endl;
                    if (bufferp2[0] == '1') {
                        requestreceived[i] = 1;
                    ...
                    else if (bufferp2[0] == '3') {
                        requestsent[i] = 0;
                    }
                    else if (bufferp2[0] == '4') {
                        replysent[i] = 0;
                    }
                }
                else {
                    if (requestreceived[i] == 0) {
                        string temp = "3";
                        int sentbytes = send(usingsocks[i], temp.c_str(), 1, 0);
                        while (sentbytes != 1) {
                            cout << "Reporting error code 3 failed to neighbour "<< neighbourids[i] << endl;
                            sentbytes = send(usingsocks[i], temp.c_str(), 1, 0);
                        }
                    }
                    else if (replyreceived[i] == 0) {
                        string temp = "4";
                        int sentbytes = send(usingsocks[i], temp.c_str(), 1, 0);
                        while (sentbytes != 1) {
                            cout << "Reporting error code 3 failed to neighbour "<< neighbourids[i] << endl;
