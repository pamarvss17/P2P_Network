    ...
    */
    ...
    for (int i = 0; i < noofneighbour; i++) {
        if (clientid < neighbourids[i]) {
            ...
            cout << "Successfully sent request " << filestoask[i] << " to " << neighbourids[i] << endl;
            int cnt = nooffilestoask[i];for (int i = 0; i < noofneighbour; i++) {
    //     if (clientid < neighbourids[i]) {
    ...
    for (int i = 0; i < noofneighbour; i++) {
        if (nbrrequeststosend[i].size() == 0) {
            cout << "Should let " << neighbourids[i] << " know that no one wants a connection with it " << endl;
        }
        else {
            cout << "Should let " << neighbourids[i] << " know that " << vectos(nbrrequeststosend[i]) << " want connections with it" << endl; 
        }
    }
