...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    vector<string> filesindirectorysorted;
    filesindirectorysorted = filesindirectory;
    sort(filesindirectorysorted.begin(), filesindirectorysorted.end());

    for (auto& it : filesindirectorysorted) 
    {
        cout << it << endl;
    }
    ...
    int wheretofind[nooffilestosearch] = {0};
    ...
    for (int i = 0; i < noofneighbour; i++) {
        for (int j = 0; j < nooffilestosearch; j++) {
            if (replyfromneighbours[i][j] == '1') {
                ...
                if (wheretofind[j] == 0) {
                    wheretofind[j] = neighbouruniqueids[i];
                }
                else {
                    wheretofind[j] = min(wheretofind[j], neighbouruniqueids[i]);
                }
            }
        }
    }
    ...
    for (int i = 0; i < nooffilestosearch; i++) {
        if (wheretofind[i] > 0) {
            cout << "Found " << filenamestosearch[i] << " at " << wheretofind[i] << " with MD5 0 at depth 1" << endl; 
        }
        else {
