...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    while(replyreceived, noofneighbour){
        for (int i = 0; i < noofneighbour; i++) {
            int bytesreceived;
            bytesreceived = recv(usingsocks[i], bufferp2, 1024, 0);
            if (bytesreceived < 0) {
                if (requestreceived[i] == 0) {
                    string temp = "0";
                    char const* temp_char = temp.c_str();
                    //while (send(usingsocks[i], temp_char, 1, 0) < 0) {
                    //}
                    send(usingsocks[i], temp_char, 1, 0);
                }
            ...
            if (requestsent[i] == 0) {
                //while (!(send(usingsocks[i], query_as_c_str, query.length(), 0) == query.length())){
                //}
                send(usingsocks[i], query_as_c_str, query.length(), 0);
                requestsent[i] = 1;
            }
            if ((requestreceived[i] == 1) && (replysent[i] == 0)) {
                string temp = "";
                ...
                for (int j = 0; j < noofwords; j++) {
                    if (searchword(wordstosearch[i], filesindirectory, nooffilesindir)) {
                        result += "1";
                    }
                    else {
                        result += "0";
                ...
                //while (!(send(usingsocks[i], result.c_str(), noofwords, 0) == noofwords)) {
                //}
                send(usingsocks[i], result.c_str(), noofwords, 0);
                replysent[i] = 1;
            }
        }
    }
