...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    while (check(checker,noofneighbour))
    {
        for (int i = 0; i < noofneighbour; i++)
        {
            if(checker[i]==0)
            {
                ...
                //     {
                //         tmp = to_string(usingsocks[i]) + " " + tmp;
                //         send(usingsocks[i] ,  tmp.c_str(), tmp.length() , 0 );
                //         valuesread[i] = recv( usingsocks[i] , buffer, 1024,0);
                //         if (valuesread[i]==6)
                //         {
                //             if (buffer[0] == neighbourids[i]) {
                //                 checker[i]=1;
                //                 // printf("%s\n",buffer );
                //                 privateids.push_back(buffer);
                //                 cout << "Accepted connection with " << neighbourids[i] << endl;
                //             }
                //             else {
                //                 cout << "Connectiomn failed with " << neighbourids[i] << endl;
                //             }
                //         }    
                //     }
                //     else {
                //         cout << "Accept failed with " << neighbourids[i] << endl;
                //     }
                // }
                // else
                // {
                //     cout << "Position " << position << endl;
                //     if (!(connect(sockfd, (struct sockaddr *)&serv_addr[i-position], sizeof(serv_addr[i-position])) < 0))
	            //     {
		                
                //         send(sockfd , num_char , 6 , 0 );
                //         valuesread[i] = recv(usingsocks[i] , buffer, 1024,0);		// server side msg reading
	            //         // printf("%s\n",buffer );
                //         if (valuesread[i]==11)
                //         {
                //             if (buffer[6] == neighbourids[i]) {
                //                 string x = "";
                //                 for (int j = 0; j < 4; j++) {
                //                     x += buffer[j];
                //                 }
                ...
                //                 privateids.push_back(buffer);
                //                 checker[i]=1;
                //                 cout << "Successfully connected with " << neighbourids[i] << endl;
                //             }
                //             else {
                //                 cout << "Connectiomn failed with " << neighbourids[i] << endl;
                //             }
	            ...
	            //     }
                //     else {
                //         cout << "Connection failed with " << neighbourids[i] << endl;
                //     }
                // }
                if (i >= position) {
    ...
    for (int i = 0; i < noofneighbour; i++)
    {
        ...
        cout<< "Connected to "<<neighbourids[i]<<" with unique-ID "<<r<<" on port "<< neighbourports[i] << " " << usingsocks[i] <<endl;
    }
    ...
    while(check(replyreceived, noofneighbour) || check(replysent, noofneighbour)){
        for (int i = 0; i < noofneighbour; i++) {
            if (requestsent[i] == 0) {
                int sentbytes = send(usingsocks[i], query_as_c_str, query.length(), 0);
                while (sentbytes != query.length()) {
                    cout << "Sending failed to neighbour "<< neighbourids[i] << endl;
                    sentbytes = send(usingsocks[i], query_as_c_str, query.length(), 0);
                }
                cout << "Request " << query << " sent to neighbour " << neighbourids[i] << endl;
                requestsent[i] = 1;
            }
            if ((requestsent[i] == 1 && replyreceived[i] == 0) || (requestreceived[i] == 0)) {
                int bytesreceived = recv(usingsocks[i], bufferp2, 1024, 0);
                cout << "Buffer " << bufferp2 << endl;
                if (bytesreceived > 0) {
                    if (bufferp2[0] == '1') {
                        requestreceived[i] = 1;
                        ...
                        replysent[i] = 1;
                        cout << "Reply " << result << "  sent to neighbour " << neighbourids[i] << endl;
                    }
                    else if (bufferp2[0] == '2') {
                        string temp = "";
                        for (int j = 2; j < bytesreceived; j++) {
                            temp += bufferp2[j];
                        }
