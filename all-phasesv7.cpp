...
int main(int argc, char* argv[])
{
    ...
    */
    ...
    for (int i = 0; i < nooffilestosearch; i++)
    {
        ...
        {
            string path = "sample-data4/files/client"+ to_string(clientid) + "/Downloaded/" + filenamestosearch[i];
                    ...
                    while (filereceiving < cnt) {
                        if (sizesoffiles[filereceiving] == 0) {
                            ...
                            if (recvsize > 0) {
                                ...
                                int remainingdata = stoi(temp);
                                string filepath  = "sample-data4/files/client" + to_string(clientid) + "/Downloaded/" + filestoask2new[i][filereceiving];
                                FILE* filepathdesc = fopen(filepath.c_str(), "w");
                                int rb = 0;
                                while (remainingdata > 0) {
